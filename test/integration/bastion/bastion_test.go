...
var _ = Describe("Bastion tests", func() {
	...
	BeforeSuite(func() {
		...
		if strings.HasPrefix(*serviceAccount, "/") {
			dat, err := ioutil.ReadFile(*serviceAccount)
			Expect(err).NotTo(HaveOccurred())
			fmt.Print(string(dat))
			*serviceAccount = string(dat)
		}
...
func createShootStruct(infrastructureConfig []byte) *gardencorev1beta1.Shoot {
...
func createClusters(name string) (*extensionsv1alpha1.Cluster, *controller.Cluster) {
	...
	shoot := createShootStruct(infrastructureConfigJSON)
...
func verifyCreation(ctx context.Context, project string, computeService *compute.Service, options *bastionctrl.Options) {
	...
	Expect(string(*createdInstance.Metadata.Items[0].Value)).To(Equal(string(userDataConst)))
}
