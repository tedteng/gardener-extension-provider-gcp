...
func validateFlags() {
	...
	if len(*region) == 0 { //todo hardcode remove this flag
...
var _ = Describe("Bastion tests", func() {
	...
	BeforeSuite(func() {
		...
		serviceAccount, err := ioutil.ReadFile("/Users/i333878/Work/gcp.json") //todo hardcode
		if err != nil {
			fmt.Print(err)
		}

		projectID, err = gcp.ExtractServiceAccountProjectID(serviceAccount) //todo hardcode serviceAccount
		Expect(err).NotTo(HaveOccurred())

		computeService, err = compute.NewService(ctx, option.WithCredentialsJSON(serviceAccount), option.WithScopes(compute.CloudPlatformScope)) //todo hardcode serviceAccount
		Expect(err).NotTo(HaveOccurred())
		region = pointer.String("europe-west1") //todo hardcode region
